...
def displayNumbersOnImage(img, init_grid, solved_grid, color = (0,150,0)):
  secW = int(img.shape[1]/9)
  secH = int(img.shape[0]/9)
  for x in range (0,9):
    for y in range (0,9):
      if init_grid[y][x] == 0:
        cv2.putText(img, str(solved_grid[y][x]), (x*secW+int(secW/2)-30, int((y+1)*secH)-30), cv2.FONT_HERSHEY_COMPLEX_SMALL, 5, color, 5, cv2.LINE_AA)
  return img